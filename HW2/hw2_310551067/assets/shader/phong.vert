...
void main() {
  ...
  vec4 tmp = modelMatrix*vec4(Position_in[0],Position_in[1],Position_in[2],1.0);
  fragPos = vec3(tmp[0],tmp[1],tmp[2]);

  tmp = normalMatrix*vec4(Normal_in[0],Normal_in[1],Normal_in[2],1.0);
  N = vec3(tmp[0],tmp[1],tmp[2]);
  LV = lightVector;
  cof = coefficients;
  ViewPos = vec3(viewPosition[0],viewPosition[1],viewPosition[2]);
  // Ambient intensity
